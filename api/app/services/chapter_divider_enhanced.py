...
"""
        ...
        try:
            ...
            if response.status_code == 200:
                # DashScope v2.0 API: 内容在 choices[0].message.content
                content = None
                ...
                if content:
                    ...
                    if json_match:
                        try:
                            ...
                            # 验证结果
                            if result.get('has_toc') and result.get('chapters'):
                ...
                print(f"❌ LLM API 错误 (状态码 {response.status_code}): {error_msg}")
                return {"has_toc": False}
    ...
    async def _create_chapter_progress(
        ...
        try:
            ...
            if 'subsections' in chapter_info and chapter_info['subsections']:
                print(f"   💾 创建 {len(chapter_info['subsections'])} 个小节记录")
                ...
                for subsection_info in chapter_info['subsections']:
                    try:
                        ...
                        print(f"      ✅ 小节 {subsection_info.get('subsection_number')} 创建成功")
                    except Exception as e:
                        print(f"      ⚠️  创建小节失败: {e}")
                        import traceback
                        traceback.print_exc()
